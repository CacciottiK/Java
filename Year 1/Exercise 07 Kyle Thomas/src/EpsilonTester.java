...
/*
 * This class can be used for practicing comparing double values with epsilon
 * It has three fields: test, target, epsilon as doubles, and has
...
public class EpsilonTester {
	...
	/*
	 * Produces a report detailing the comparison of target and test numbers using
	 * the difference of the target and test versus epsilon.
	 */
	public String compareWithEpsilon() {
		boolean result = Math.abs(target - test) <= epsilon;
		String header = String
				.format("Testing with: Math.abs(target - test) <= epsilon%n" + "value of epsilon is: %.14f", epsilon);
		String report = generateReport(header, result);
		return report;
	}

	/*
	 * This method interacts with a user at a console window to prompt for, and
	 * accept values for the fields. Inputs are assigned to the fields immediately.
	 */
	public void interactWithUserForFieldValues() {
		double value = 0.0; // used for target, test, epsilon in sequence
	...
	/*
	 * This method generates a report based on the header-line, and a boolean value
	 * representing the result of a comparison.
	 * 
	 * Reminder when using either: printf(formatString, data_1, ... , data_n)
	 * String.format(formatString, data_1, ... , data_n) %f is for float or double,
	 * %.14f means show 14 decimal places %b is for boolean %n is for newline %d is
	 * for integers (not used here) %s is for String,
	 */
	private String generateReport(String header, boolean result) {
		...
		report += String.format("%s %.14f%n", "absolute difference: ", Math.abs(target - test));
